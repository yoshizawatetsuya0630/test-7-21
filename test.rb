...
def 寄付金控除(所得, 寄付金, 資本金)
    控除1 = 控除1計算(所得, 寄付金, 資本金)
    控除2 = 控除2計算(所得, 寄付金, 資本金)
    控除3 = 控除3計算(所得, 寄付金, 資本金)

    控除1 + 控除2 + 控除3
end

def 控除1計算(所得, 寄付金, 資本金)
    #法人住民税の2割、寄付金の4割　いずれか少ない方
    法人住民税分 = [法人住民税計算(所得, 寄付金, 資本金) * 0.2, 寄付金 * 0.4]
    法人住民税分.min
end

def 控除2計算(所得, 寄付金, 資本金)
    #法人税の5%、寄付金の1割、法人住民税で4割に達しなかった場合の補填額　いずれか少ない方
    法人税等分 = [(法人税計算(所得, 寄付金, 資本金)+ 地方法人税計算(所得, 寄付金, 資本金)) * 0.05, 寄付金 * 0.1, (寄付金 * 0.4 - 法人住民税計算(所得, 寄付金, 資本金))]
    法人税等分.min
end

def 控除3計算(所得, 寄付金, 資本金)
    #法人事業税の2割、　寄付金の2割　いずれか少ない方
    法人事業税分 = [法人事業税計算(所得, 寄付金, 資本金) * 0.2, 寄付金 * 0.2]
    法人事業税分.min
end

寄付金損金算入効果 = 法人税等合計(100_000_000, 0 ,100_000_001) - 法人税等合計(100_000_000, 20_000_000, 100_000_001)
自己負担額 = 20_000_000 - 寄付金控除(100_000_000, 20_000_000, 100_000_001) - 寄付金損金算入効果
自己負担割合 = 自己負担額 / 20_000_000 * 100

puts "法人税等合計　#{法人税等合計(100_000_000, 20_000_000, 100_000_001).round}"
puts "寄付金控除額合計 #{寄付金控除(100_000_000, 20_000_000, 100_000_001).round}"
